...
## [P-009] JavaScript `\b` word boundary가 한국어(CJK)에서 작동하지 않음

- **발견**: 2026-03-11 (감사 관찰 사항 #2 수정 중)
- **증상**: `/\b(세금|소상공인|AI)\b/i.test('소상공인을 위한 AI 세금 앱')` → `false`
- **원인**: JavaScript 정규식의 `\b`는 `\w`(영문+숫자+_)와 `\W` 사이에서만 작동. 한국어는 `\W`이고 공백도 `\W`이므로 한국어 단어 앞뒤에서 word boundary가 인식되지 않음
- **해결**: 한국어 키워드가 포함된 regex에서 `\b` 제거. 영문 전용 패턴(예: `Next.js`, `Firebase`)만 `\b` 유지
- **재발 방지**: **한국어가 포함된 regex에 `\b` 절대 사용 금지**. `(?<=\s|^)` / `(?=\s|$)` 또는 그냥 loose match 사용

*마지막 업데이트: 2026-03-11 | v0.7.5*
